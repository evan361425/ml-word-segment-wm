...
class CRF(nn.Module):
    def __init__(
        self, tagset_size, gpu, use_predefined_matrix=False, transition_matrix=None
    ):
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        partition = (
            inivalues[:, START_TAG, :].clone().view(batch_size, tag_size, 1)
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        # only need start from start_tag
        partition = (
            inivalues[:, START_TAG, :].clone().view(batch_size, tag_size)
        )  # bat_size * to_target_size
