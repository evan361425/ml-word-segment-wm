...
def train(args):
    ...
    if args.use_memory:
        if args.ngram_num_threshold <= 1:
            raise Warning(
                "The threshold of n-gram frequency is set to %d. "
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, start_index in enumerate(
                ...
                batch_examples = train_examples[
                    start_index : min(
                        start_index + args.train_batch_size, len(train_examples)
                    )
                ]
                ...
                (
                    ...
                    word_ids,
                    word_mask,
                ) = feature2input(device, train_features)
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                ...
                for start_index in range(0, len(eval_examples), args.eval_batch_size):
                    eval_batch_examples = eval_examples[
                        start_index : min(
                            start_index + args.eval_batch_size, len(eval_examples)
                        )
                    ]
                ...
                if f > best_f:
                    ...
                    if args.model_name:
                        with open(
                            os.path.join(output_model_dir, "CWS_result.txt"), "w"
                        ) as writer:
                            for i in range(len(y_pred)):
                                sentence = eval_examples[i].text_a
                                seg_true_str, seg_pred_str = eval_sentence(
                        ...
                        best_eval_model_path = os.path.join(
                            output_model_dir, "model.pt"
                        )
        ...
        if os.path.exists(output_model_dir):
            with open(
                os.path.join(output_model_dir, "history.json"), "w", encoding="utf8"
            ) as f:
                json.dump(history, f)
                f.write("\n")
...
def test(args):
    ...
    seg_model_checkpoint = torch.load(args.eval_model, map_location=device)
    ...
    for start_index in tqdm(range(0, len(eval_examples), args.eval_batch_size)):
        eval_batch_examples = eval_examples[
            start_index : min(start_index + args.eval_batch_size, len(eval_examples))
        ]
        eval_features = convert_examples_to_features(eval_batch_examples)
...
def predict(args):
    ...
    print(
        "device: {} gpu#: {}, distributed training: {}, 16-bits training: {}".format(
            device, n_gpu, bool(args.local_rank != -1), args.fp16
        )
    )
...
def main():
    ...
    parser.add_argument(
        "--do_predict", action="store_true", help="Whether to run training."
    )
    parser.add_argument(
        "--train_data_path",
        default=None,
    ...
    parser.add_argument("--use_zen", action="store_true", help="Whether to use ZEN.")
    parser.add_argument(
        "--bert_model",
        default=None,
        type=str,
    ...
    parser.add_argument("--eval_model", default=None, type=str, help="")
    parser.add_argument(
        "--cache_dir",
        default="",
        type=str,
    ...
    parser.add_argument(
        "--patient", type=int, default=3, help="Patient for the early stop."
    )
    parser.add_argument(
        "--ngram_num_threshold",
        type=int,
        default=0,
        help="The threshold of n-gram frequency",
    )
    ...
    parser.add_argument(
        "--use_memory", action="store_true", help="Whether to run training."
    )
    parser.add_argument(
        "--decoder",
        type=str,
    ...
    if args.do_train:
        ...
        raise ValueError(
            "At least one of `do_train`, `do_eval`, `do_predict` must be True."
        )
